...
//paging
	...
	$result_count = $db->query("SELECT COUNT(*) As total_records FROM tr_pesan WHERE id_user_tujuan = '$_SESSION[id_tb_user]' and st_hapus_tujuan = '1' and st_pesan = '1' ");
	$total_records = $db->fetchArray($result_count);
	$total_records = $total_records['total_records'];
	$total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

	$sql = "SELECT * FROM tr_pesan WHERE id_user_tujuan = '$_SESSION[id_tb_user]' and st_hapus_tujuan = '1' and st_pesan = '1' order by tgl_data desc, st_bintang OFFSET $offset ROWS FETCH NEXT $total_records_per_page ROWS ONLY";
	$res = $db->query($sql);
          ...
          <div class="box box-solid">
            ...
            <div class="box-body no-padding">
              <ul class="nav nav-pills nav-stacked">
                <li class="active"><a href="pesan_v.php"><i class="fa fa-inbox"></i> Inbox
				  <? $sql2 = $db->fetchArray($db->query("select count(id_tr_pesan) as jmlpesan from tr_pesan where st_baca = '1' and st_pesan = '1' and id_user_tujuan = '$_SESSION[id_tb_user]' and st_hapus_tujuan = '1'"));					  
				  if($sql2['jmlpesan'] > 0){
				  ?>
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
                  ...
                  <tr <?if($row['st_baca'] == 1) {echo 'style="background-color: #e5f5ff;"';}?>>
                    ...
                    <td class="mailbox-name"><a href="pesan_baca.php?id=<?=maxiline($row['id_tr_pesan'], 'e');?>&p=i"><?=$sql1['nm_user'];?></a></td>
                    <td class="mailbox-subject"><b><a style="color:black;" href="pesan_baca.php?id=<?=maxiline($row['id_tr_pesan'], 'e');?>&p=i"><?=$row['subyek'];?></b> - <?=substr(strip_tags(htmlspecialchars_decode($row['pesan'])), 0, 65);;?>...</a>
                    </td>

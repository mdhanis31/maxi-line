  ...
  <div class="content-wrapper">
    ...
    <section class="content">
          ...
          <div class="row">
            <div class="col-md-6">
              ...
              <div class="form-group">
				...
				<input class="form-control" id="namakustomer" value="<?=$rowa['nama'];?>" disabled>				
				</div>
              </div>
              </div>
			   <div class="form-group">
				...
				<input class="form-control" id="alamatkustomer" value="<?=$rowa['alamat'];?>" disabled>				
				</div>
              </div>
              </div>
			   <div class="form-group">
            ...
            <div class="col-md-6">
              ...
              <div class="form-group">
				...
				<input class="form-control" id="emailkustomer" value="<?=$rowa['email'];?>" disabled>				
				</div>
              </div>
			    <div class="form-group">
                <div class="input-group">
				...
				<input class="form-control" id="telpkustomer" value="<?=$rowa['telp'];?>" disabled>				
				</div>
              </div>
			     <div class="form-group">
                <div class="input-group">
      ...
      <!-- /.box -->
			...
			<?
			if($ro['sts_lunas'] == 1 or $ro['sts_lunas'] == 3) {?>
			<button class="btn btn-info" id="bayar" >Bayar</button>
			<?}?>
...
$(document).on('click', '#bayar', function (e) {
    ...
    let data = {
        nameinv: '<?=$ro['no_invoice'];?>',
        customerName: '<?=str_replace(array("\r", "\n"), " ", $rowa['nama']);?>',
        email: '<?=$rowa['email'];?>',
        phoneNumber: '<?=$rowa['telp'];?>',
        address: '<?=str_replace(array("\r", "\n"), " ", $rowa['alamat']);?>',
        country: 'ID',
        expiredTime: 60,
        amount: <?=$ro['tot_tagih'];?>,
		id_tb_pendaftaran: <?=$id_tb_pendaftaran;?>,
		id_tr_invoice: <?=$id_tr_invoice;?>
    };
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result && result.internal_request_id) {
                let internalId = result.internal_request_id;
                window.currentInternalId = internalId; // Save globally for cancellation listener
                
                if (result.response && result.response.payment && result.response.payment.url) {
                    // Start Jokul Checkout
                    loadJokulCheckout(result.response.payment.url);
                    
                    // Detection for cancellation (X click)
                    let modalOpened = false;
                    let checkInterval = setInterval(function() {
                        // Check if DOKU modal or iframe is present AND visible
                        let modalElement = $('#jokul-checkout-modal');
                        if (modalElement.length === 0) modalElement = $('#jokul_checkout_modal'); // User mentioned underscore
                        
                        let modalPresent = modalElement.length > 0;
                        let modalVisible = modalElement.is(':visible');
                        
                        if (modalPresent && modalVisible) {
                            modalOpened = true;
                        }

                        if (modalOpened && (!modalPresent || !modalVisible)) {
                            ...
                            // We wait a bit to see if a redirect happens
                            setTimeout(function() {
                                if (typeof window.paymentSuccess === 'undefined' || !window.paymentSuccess) {
                                    updateStatus(internalId, 3, "User membatalkan proses pembayaran", function() {
                                        location.reload();
                                    });
                                } else {
                                    Swal.close();
                    ...
                    updateStatus(internalId, 3, "Tidak mendapatkan URL pembayaran");
                    Swal.fire({
                        icon: 'error',
                        title: 'Payment Error',
                        text: 'Tidak mendapatkan URL pembayaran.',
                 ...
                 Swal.fire({
                    icon: 'error',
                    title: 'System Error',
                    text: 'Gagal inisialisasi transaksi internal.',
                    confirmButtonText: 'Tutup',
                });
            }
        },
        error: function(xhr, textStatus, error){
            // If we have access to the response, we might find the internal_request_id
            let res = {};
            try { res = JSON.parse(xhr.responseText); } catch(e) {}
            
            if (res.internal_request_id) {
                updateStatus(res.internal_request_id, 3, "Terjadi kesalahan saat menghubungi server");
            }
...
$(document).on('click', '.fa-window-close', function() {
    // If we can find the internalId, we update it
    if (window.currentInternalId) {
        Swal.fire({
            title: 'Memproses...',
            allowOutsideClick: false,
            didOpen: () => {
        ...
        updateStatus(window.currentInternalId, 3, "User membatalkan proses pembayaran", function() {
            location.reload();
        });
    }
});

function updateStatus(id, status, reason, callback) {
    $.ajax({
        type: "POST",

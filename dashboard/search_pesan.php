...
echo " <script>alert('Kata kunci tidak boleh kosong!');</script>
	   <script>history.back();</script>";
		exit();
}

$kunci = SafeSQL($_POST['kunci']);	
...
//paging
	...
	$result_count = $db->query("SELECT COUNT(*) As total_records (SELECT * FROM v_pesan WHERE id_user_tujuan = '$_SESSION[id_tb_user]' or id_user_pengirim = '$_SESSION[id_tb_user]')a where pesan like '%$kunci%' or subyek like '%$kunci%'  or pengirim like '%$kunci%' or tujuan like '%$kunci%' or kd_tujuan like '%$kunci%' or kd_pengirim like '%$kunci%'");
	$total_records = $db->fetchArray($result_count);
	$total_records = $total_records['total_records'];
	$total_no_of_pages = ceil($total_records / $total_records_per_page);
	$second_last = $total_no_of_pages - 1; // total page minus 1

	$sql = "SELECT * FROM (SELECT * FROM v_pesan WHERE id_user_tujuan = '$_SESSION[id_tb_user]' or id_user_pengirim = '$_SESSION[id_tb_user]')a where pesan like '%$kunci%' or subyek like '%$kunci%'  or pengirim like '%$kunci%' or tujuan like '%$kunci%' or kd_tujuan like '%$kunci%' or kd_pengirim like '%$kunci%' order by tgl_data desc, st_bintang OFFSET $offset ROWS FETCH NEXT $total_records_per_page ROWS ONLY";
	$res = $db->query($sql);
          ...
          <div class="box box-primary">
            ...
            <div class="box-body no-padding">
              <div class="mailbox-controls">
				...
				<button type="button" class="btn btn-default btn-sm" onclick="window.location.href='pesan_v.php'"><i class="fa fa-reply"></i></button>
                <div class="btn-group">				
                  <button type="button" class="btn btn-default btn-sm tombolhapus" name="hapus" ><i class="fa fa-trash-o"></i></button>
                </div>
                <!-- /.btn-group -->
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
				  ...
				  if(empty($total)){?>
				   <tr>
                   <td>Hasil pencarian tidak ditemukan</td>
				   </tr>  
				  <?} else {
					while($row = $db->fetchArray($res)) {
				  ?>
                  <tr <?if($row['st_baca'] == 1) {echo 'style="background-color: #e5f5ff;"';}?>>
                    <td><input type="checkbox" name="opsi[]" id="pilih" value="<?=$row['id_tr_pesan'];?>"><input type="hidden" name="id_tr_pesan[]" value="<?=$row['id_tr_pesan'];?>" /></td>
                    <td class="mailbox-star"><a href="#" class="star_hapus" value="<?=$row['id_tr_pesan'];?>" id2="y">
					<?
					if($row['st_bintang'] == 2) {
					?>
					<i class="fa fa-star text-yellow">
				    <?} else if($row['st_bintang'] == 1) {?>
					<i class="fa fa-star-o text-yellow ">
					<?} else if($row['st_hapus_tujuan'] == 2 or $row['st_hapus_pengirim'] == 2) {}?>
					</i></a></td>
                    <td class="mailbox-name"><a href="pesan_baca.php?id=<?=maxiline($row['id_tr_pesan'], 'e');?>&p=b">
					<?
					...
					$sql1 = $db->fetchArray($db->query("select * from tb_user where id_tb_user = '$row[id_user_tujuan]'"));	
					$namanya = "To : ".$sql1['nm_user'];
					}	
					echo "$namanya";
					?>
					</a></td>
                    <td class="mailbox-subject"><b><a style="color:black;" href="pesan_baca.php?id=<?=maxiline($row['id_tr_pesan'], 'e');?>"><?=$row['subyek'];?></b> - <?=substr(strip_tags(htmlspecialchars_decode($row['pesan'])), 0, 65);;?>...</a>
                    </td>
					<td>
					<?
					if($row['st_hapus_tujuan'] == 2 or $row['st_hapus_pengirim'] == 2) {echo "Thrash";} else if($row['st_pesan'] == 2 and $row['id_user_pengirim'] == $_SESSION['id_tb_user']) {echo "Drafts";} else if($row['id_user_tujuan'] == $_SESSION['id_tb_user'] and $row['st_hapus_tujuan'] == 1) {echo "Inbox";} else if($row['id_user_pengirim'] == $_SESSION['id_tb_user'] and $row['st_hapus_pengirim'] == 1)
					{echo "Sent";}
					?>
					</td>

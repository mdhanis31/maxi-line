...
if(!isset($_POST['searchTerm'])){ 
  $sql = $db->query("select top 100 * from tb_user where id_tb_pendaftaran is not null");   
}else{ 
  $search = $_POST['searchTerm'];   
  $sql = $db->query("select * from tb_user where id_tb_pendaftaran is not null and nm_user like '%$search%' or kode_daftar like '%$search%'");
} 

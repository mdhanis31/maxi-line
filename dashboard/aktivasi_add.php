...
$sql = "select top 1 * from tb_rencana where id_tb_pendaftaran = '$id_tb_pendaftaran' and rencana = '3' and st_rencana = '1' order by tgl_rencana asc";
$res = $db->query($sql);
$row = $db->fetchArray($res);
...
$tglaktivasi = date_format($row['tgl_rencana'], 'd-m-Y');	
$jamaktivasi = date_format($row['tgl_rencana'], 'H:i');
}
}
      ...
      <div class="row">
          ...
          <div class="box box-primary">
			...
			<?}?>
              ...
              <div class="box-footer" id="olaole">
				 ...
				 <a href="aktivasi_add.php?id=<?=maxiline($id_tb_aktivasi, 'e');?>&s=e" class="btn btn-primary">Revisi</a> 
				<?} else if($_GET['s']=='d' and $_GET['t']=='u') {?>
				 <a href="aktivasi_add.php?id=<?=maxiline($id_tb_aktivasi, 'e');?>&s=e&t=u" class="btn btn-primary">Revisi</a> 				 
				...
				if($_GET['s']=='b') {
					...
					<a href="aktivasi_add.php?id=<?=maxiline($id_tb_aktivasi, 'e');?>&s=e&t=u" id="olaole" class="btn btn-primary">Revisi</a>
					<?}
				} if($_GET['s']=='d') {
					if($row['sts_aktivasi'] == 1){?>
					<a href="aktivasi_add.php?id=<?=maxiline($id_tb_aktivasi, 'e');?>&s=e" id="olaole" class="btn btn-primary">Revisi</a> 	
					<?}
				} else if($_GET['s']=='e') {
					if($row['sts_aktivasi'] == 1){
						if(in_array($_SESSION['id_tb_user'], $id_usere) or $_SESSION['level_user'] == '1') {?>
				...
				if($_GET['t']=='u') {?>
				<button type="reset" class="btn btn-default" style="margin-left:5px;" onclick="window.location.href='aktivasi_v.php'">Cancel</button>
				<?} else {?>
		...
		<div class="box box-primary">
            <div class="box-header with-border">
			  ...
			  <div class="box-tools">
				<?php if ($row['sts_aktivasi'] != 2) {?>
						<a href="laporan_aktivasi_add.php?id=<?=maxiline($id_tb_aktivasi, 'e');?>&s=b&token=<?echo $kodeaman;?>" class="btn btn-sm btn-primary" style="margin-top:1px;"> 
				...
				?>
					<li class="item">
						...
						<div class="product-info">
						  <a href="laporan_aktivasi_add.php?id=<?=maxiline($rowx['id_tb_laporan'], 'e');?>&s=e" class="product-title"><?=$sqlz['nm_user']?>

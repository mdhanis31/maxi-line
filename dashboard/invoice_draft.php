...
$sql = "SELECT * FROM tr_invoice WHERE sts_invoice = '1' order by tgl_invoice desc";
$res = $db->query($sql);
    ...
    <!--        <div class="box-header">
            ...
            <div class="box-body border-radius-none">
              ...
              <table id="example1" class="table table-bordered table-striped">
				...
				$sql1 = "SELECT * FROM tr_transaksi WHERE id_tr_invoice = '$row[id_tr_invoice]'";
				$jml1 = $db->queryNumRows($sql1);
				$total1 = $db->getNumRows($jml1);
												
				$n++;?>	
				 <tr>
                  <td><?=$n;?></td>
                  <td><?if(!empty($total1)) {echo $sqla['nama'];} else {echo "<span style=\"color:red;\">".$sqla['nama']."</span>";}?></td>
                  ...
                  <ul class="dropdown-menu dropdown-menu-right" role="menu">                   
                    <li><a href="invoice_add<?if($row['jns_invoice'] != 1){echo $row['jns_invoice'];}?>.php?id=<?=maxiline($row['id_tb_pendaftaran'], 'e');?>&i=<?=maxiline($row['id_tr_invoice'], 'e');?>&j=<?=$row['jns_invoice'];?>">Edit</a></li>
					<?
					if($_SESSION['level_user'] == 1) {?>
                    <li class="divider"></li>
                    <li><a href="invoice_.php?id=<?=maxiline($row['id_tr_invoice'], 'e');?>&h=y" onclick="return confirm('Hapus invoice?')">Hapus</a></li>
					<?}?>
                  </ul>
                </div> 
				  </td>		

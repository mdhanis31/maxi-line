...
if($_GET['s'] == "d" or $_GET['s'] == "e") { 
	...
	$id_tb_perawatan = $rowu['id_jns_laporan'];
	...
	$tgllaporan = date_format($row['tgl_perawatan'], 'd-m-Y');	
    ...
    <section class="content">
		...
		<section class="col-lg-12">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Laporan Perawatam / Perbaikan</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
			...
			<?}?>
				...
				<div class="form-group">
                 ...
                 <select class="form-control select2" data-placeholder="pilih hasil perawatan" style="width: 100%;" disabled> 
				 <?if($_GET['s'] == "b") {?>
				 <option value="">--- Pilih Hasil ---</option>
				 <?}?>
				 <option value="1" <?if($row['st_perawatan'] == 1) {echo "selected";}?> >Direview</option>
				 <option value="2" <?if($row['st_perawatan'] == 2) {echo "selected";}?> >Disetujui</option>
				 <option value="3" <?if($row['st_perawatan'] == 3) {echo "selected";}?> >Ditolak</option>
				</select>
                </div>
				</div>
				</div>
				...
				<div class="form-group">
				  ...
				  <?} else if($_GET['s']=='e') {?>
				  <textarea type="textarea" class="form-control" id="keterangan" name="isi_laporan" <? if ($row['sts_perawatan'] != 2) {echo "required";} else {echo "disabled";}?> placeholder="Masukkan laporan" ><?=$rowu['isi_laporan'];?></textarea>
				  <?} else {?> 
				...
				if($row['sts_perawatan'] == 1) {
					if($_GET['s']=='e') {
						if($rowu['id_tb_user']== $_SESSION['id_tb_user']) {?>
						 <button type="submit" id="olaole" class="btn btn-primary">Simpan</button>	
						<?} else if($_GET['s']=='e') {?>						
						 <button type="submit" id="olaole" class="btn btn-primary">Simpan</button>
						<?}
					} else {	
	  ...
	  <div class="modal-dialog">
		<div class="modal-content">
	      ...
	      <input type="hidden" name="id_tb_proses" value="<?=$id_tb_perawatan?>">
		  <input type="hidden" name="st_proses" value="4">
		  <input type="hidden" name="id_tb_pendaftaran" value="<?=$id_tb_pendaftaran?>">

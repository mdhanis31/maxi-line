							...
							<div class="table-responsive">
								<table id="example1" class="table table-bordered table-striped">
									...
									<?php if($_SESSION['level_user'] == 6) {
										...
										// Get Pendaftaran by Reseller
										$st_layanan = [8,9,10];
										$list_layanan = implode(",", $st_layanan);
										$qry1 = $db->query("SELECT * FROM tb_pendaftaran WHERE st_layanan IN ($list_layanan) AND referral_by = '$row[referral]'");
									?>

										<?php while ($res = $db->fetchArray($qry1)) { ?>
											<?php
												$qry2 = $db->query("SELECT df.nama, inv.* FROM tr_invoice inv RIGHT JOIN 
															tb_pendaftaran df ON df.id_tb_pendaftaran = inv.id_tb_pendaftaran
															WHERE inv.id_tb_pendaftaran = '$res[id_tb_pendaftaran]' AND sts_invoice = 2
															ORDER BY tgl_data DESC"
														);
												// select * from tr_invoice where id_tb_pendaftaran = '$sql[id_tb_pendaftaran]' and sts_invoice = '2' order by tgl_data desc

												while($data = $db->fetchArray($qry2)) {
											?>
												<tr>
													<td><?= $n++; ?></td>
													<td><?= $data['nama'] ?></td>
													<td><?= $data['no_invoice'] ?></td>
													<td><?= tglindo(date_format($data['tgl_invoice'], 'Y-m-d')); ?></td>
													<td><?= $jns_potongan[$data['jns_invoice']]; ?></td>				  
													<td><?= $sts_lunas[$data['sts_lunas']]; ?></td>
													<td>
														<a class="btn btn-info btn-sm" href="invoice_pelanggan_add.php?id=<?=maxiline($data['id_tb_pendaftaran'], 'e');?>&i=<?=maxiline($data['id_tr_invoice'], 'e');?>&j=1">
															Lihat Detail
														</a>
													</td>
												</tr>

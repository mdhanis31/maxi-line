 ...
 */
require_once('SystemComponent2.php');
class DbConnector2 extends SystemComponent2 {
	var $theQuery;
	var $link;
	...
	function __construct() {
		...
		$settings = SystemComponent2::getSettings();
...
//		$this->link = sqlsrv_connect($host, $connInfo) === null ? die("<center><font color=red><BR /><BR /><BR /><BR />MOHON MAAF !!!<BR />DATABASE WEBSITE PUSAT KARANTINA IKAN SEDANG DALAM PERBAIKAN..<BR />MOHON MAKLUM</font></center>") :  sqlsrv_connect($host, $connInfo);
		...
		$this->link = sqlsrv_connect($host, $connInfo) or //header('Location:maintenance.php'); 	
		
		die("<center><font color=red><BR /><BR /><BR /><BR />MOHON MAAF !!!<BR />DATABASE SEDANG DALAM PROSES OPTIMASI</font></center>");
	...
	function queryNumRows($query) {
		$this->theQuery = $query;
		return sqlsrv_query($this->link, $query, array(), array("Scrollable"=>"buffered"));
	}		

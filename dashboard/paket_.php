...
$id_tb_lokasi = $_POST['id_tb_lokasi'];
...
if ($kodeaman && $_POST['token']==$kodeaman) {
    ...
    ?><script>
      alert('Area coverage tidak boleh kosong!');
      history.back();
      </script><?php
	  exit();
	} else {
	...
	$sqla ="insert into tb_paket (nama_paket, harga_paket, isi_paket, paket_promo, id_paket_utama, is_hidden, id_tb_lokasi) values ('$nama_paket', '$harga_paket', '$isi_paket', '$paket_promo', '$id_utama', '$is_hidden','$id_tb_lokasi')";
	$resa =  $db->query($sqla);
...
//---------------------------------	
	...
	$sqla = $db->query ("update tb_paket set nama_paket='$nama_paket', harga_paket='$harga_paket', isi_paket='$isi_paket', paket_promo='$paket_promo', id_paket_utama='$id_utama', is_hidden='$is_hidden', id_tb_lokasi='$id_tb_lokasi' where id_tb_paket='$id_tb_paket'");
	// insert database
	if ($sqla) {	 

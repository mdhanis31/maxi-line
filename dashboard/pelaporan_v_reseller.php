						...
						<div class="box-body">
							<div class="table-responsive">
								<table id="example1" class="table table-bordered table-striped">
									...
									$qrya = $db->query("SELECT * FROM tr_pelaporan pl LEFT JOIN tb_pendaftaran df ON pl.id_tb_pendaftaran = df.id_tb_pendaftaran WHERE df.referral_by = '$row[referral]'");

									while ($data = $db->fetchArray($qrya)) {
									?>
										<tr>
											<td><?= $n; ?></td>
											<td><?= $data['kode_daftar']; ?></td>
											<td><?= $data['nama']; ?></td>
											<td><?= tglindo(date_format($data['tgl_data'], 'Y-m-d')); ?></td>
											<td><? if (empty($data['tgl_close'])) {
													echo "-";
												} else {
													echo tglindo(date_format($data['tgl_close'], 'Y-m-d'));
												} ?></td>
											<td><?= $data['subyek_laporan']; ?></td>
											<td><?= $jns_laporan[intval("0" . $data['jns_laporan'])]; ?></td>
											<td>
												<div class="label label-<? if ($data['sts_laporan'] == 1) {
																			echo "danger";
																		} else if ($data['sts_laporan'] == 2) {
																			echo "warning";
																		} else if ($data['sts_laporan'] == 3) {
																			echo "success";
																		} ?>">
													<?= $sts_laporan[intval("0" . $data['sts_laporan'])]; ?>
												</div>
											</td>
											<?php if ($_SESSION['level_user'] != 5) { ?>
												<td>
													<?php
													if (empty($data['tgl_close'])) {
														$datetime1 = $data['tgl_data'];
														$datetime2 = $now;
											...
											<?php } ?>
											<td>
												<a class="btn btn-info btn-sm" href="pelaporan_add.php?id=<?= maxiline($data['id_tr_pelaporan'], 'e'); ?>&i=<?= maxiline($data['code_pelaporan'], 'e') ?>">Detail</a>
											</td>
										</tr>
									<?php } ?>
								</table>

...
$sql = "select * from tb_tos where id_tb_tos='$id'";
$res = $db->query($sql);
        ...
        <!-- Left col --> 		
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title"> <?if ($_GET['a']=='b') { echo "Edit Term of Service";} else if ($total=='0') { echo "Input Term of Service";} else { echo "Detail Term of Service";}?></h3>
            </div>
            <!-- /.card-header -->            
			<form action="tos_.php" method="post" enctype="multipart/form-data" name="form" id="form" role="form" onsubmit="return validateSearch()">
			 <?if ($_GET['a']=='b') {?>
			 <input type="hidden" name="j" value="b" />
			 <?} else if($total=='0') {?> 
			 <input type="hidden" name="j" value="a" />
			 <?}?>
			 <input type="hidden" name="id_tb_tos" value="<?php echo $row['id_tb_tos'];?>">						

...
.float-sm-right {
		...
		<div class="col-md-9">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title"><? if ($_GET['a']=='b') {echo "Edit";} else {echo "Tambah";}?> User</h3>
			...
			<form action="user_.php" method="post" enctype="multipart/form-data" name="form" id="form" role="form">
			<?if($_GET['a'] == 'b') {?>
			...
			<div class="box-body">
					...
					<?
					$sqlb = "select * from tb_level";
					$resb = $db->query($sqlb);
					while($rowb = $db->fetchArray($resb)){
					?>
					<option value="<?=$rowb['id_tb_level'];?>" <?if($rowa['level_user'] == $rowb['id_tb_level']){echo "selected";}?>><?=$rowb['nm_lvl'];?></option>
					<?}?>
					</select>
					</div>
					<?} else {?>
					...
					<?if($_GET['a']=='b') {?>
					  ...
					  <button type="reset" class="btn btn-default" onclick="window.location.href='user_v.php'">Batal</button>					 
					<?} else {?>
		...
		<div class="col-md-3">
			...
			<div class="box box-primary">
				<div class="box-body box-profile">
					<?php if(empty($rowa['pasfoto'])){$foto = "icon.jpg";} else {$foto = $rowa['pasfoto'];}?>

					<img class="profile-user-img img-responsive img-circle" src="dist/foto_profil/<?=$foto?>" alt="User Image" >

					<div class="col-md-12" style="text-align:center;margin-bottom: 15px">
						<a data-toggle="modal" href="#" data-target="#defaultModal1" class="btn btn-primary" style="margin-top: 15px;">Rubah Foto</a>
					</div>
					<?php if ($rowa['level_user'] == 6) { ?>
						<?php $userLvl = [1,4] ?>
						<?php if(empty($rowa['referral']) && in_array($_SESSION['level_user'], $userLvl)) : ?>
							<!-- Generate Kode Referral -->
							<div class="row">
								<div class="col-md-12">
									<label for="refferal">Generate Referrral</label>
									<div class="input-group">
										<input type="text" style="font-size:large; font-weight: bold;" class="form-control" id="referral" placeholder="Kode Referral" readonly>
										<span class="input-group-btn">
											<button class="btn btn-default" id="gen-code" type="button" data-id="<?= $_GET['i'] ?>" data-token="<?= $kodeaman ?>"><i class="fa fa-refresh" aria-hidden="true"></i></button>
										</span>
									</div><!-- /input-group -->
								</div>
							</div>
						<?php elseif(!empty($rowa['referral']) || $_SESSION['level_user'] == 6) : ?>
							<!-- Kode Referral -->
							<div class="row">
								<div class="col-md-12">
									<label for="refferal">Kode Referrral</label>
									<div class="input-group">
										<input type="text" style="font-size:large; font-weight: bold;" class="form-control" placeholder="Kode Referral" value="<?= $rowa['referral']; ?>" readonly>
										<span class="input-group-btn">
  ...
  <!-- /.content-wrapper -->
               ...
               <div class="modal-dialog" role="document" >			
				<div class="modal-content">
					...
					<div class="modal-body">							
					 ...
					 <!-- form start -->
						<form action="user_.php" method="post" enctype="multipart/form-data" name="form" id="form2" role="form">
    ...
    <strong>Copyright &copy; <?= date('Y'); ?> Maxi-Line.</strong> All rights
    reserved.
  </footer>
</div>
<!-- ./wrapper -->
...
<script>
	...
	genButton.on('click', function(){
		var token = xs(this).data('token');
		var id = xs(this).data('id');

		xs.ajax({
			url: 'gen_referral.php', // URL FILE PHP UNTUK PROSES GENERATE
            type: 'POST',
            dataType: 'json',
			data: {
				id: id,
			...
			success: function(res) {
				...
				Swal.fire({
					...
					if (result.dismiss === Swal.DismissReason.timer) location.reload()
				});
            },
		})
	});

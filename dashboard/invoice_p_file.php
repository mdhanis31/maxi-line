...
function createpdf($id_tr_invoice) {
...
<body>
		<header>
			...
			<p><img alt="" src="dist\img\MaxiLine.png" style="width:30%; margin-top:15px;"></p>
			</td>
			...
			<table style="width:80%;">
				...
				<tr class="meta">
					...
					<td class="meta" style="font-size:12px;">'.jnzinvoice($ro['jns_invoice']).'</td>
				</tr>
				<tr class="meta">
				...
				<tr class="meta">
					...
					<td class="meta" style="font-size:12px;">'.statuslunaz($ro['sts_lunas']).'</td>
				</tr>
			</table>
			</td>
			</tr>
		...
		<td style="border: 0px solid black;padding:0px;margin:0px;width: 50%;float: right;" valign="top">
			...
			<table align="center" class="inventory" style="width:95%;">
				...
				if($ro['jns_invoice'] == 1 or $ro['jns_invoice'] == 4 and $row1['jns_transaksi'] == 3 and $sqlb['harga_paket'] != $row1['sub_total']){
	...
	$isi.= '	<tr>
		...
		<aside>
			...
			<div >
			<table style="width:90%; border: 0px solid black; border-collapse: collapse;" cellspacing="0" cellpadding="0" align="center">
			<tr>
			<td style="width:90%; border: 0px solid black; border-collapse: collapse;">
			<ul>';
			 $sqls = "SELECT * from tb_note";
		     $ress = $db->query($sqls);
		     while($rows = $db->fetchArray($ress)) {
				
	$isi.=   '<li style=" margin: 10px 0;font-size:11px;"><strong>'.$rows['judul'].' : </strong>'.$rows['note'].'</li>';              
			}
 ...
 ';  
    ...
    $mpdf->Output("invoice_tagihan/".$judul.".pdf" ,'F');	
    //exit;
	$status = 2;
}
	return $status;

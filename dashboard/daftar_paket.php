...
if(!isset($_POST['searchTerm'])){	
	...
	if($id_lokasi == 4) {
		...
		$sql = $db->query("SELECT * FROM tb_paket WHERE ',' + id_tb_lokasi + ',' LIKE '%,$id_lokasi,%' and is_hidden = '1'"); 
	}	
	...
	if($id_lokasi == 4) {
		...
		$sql = $db->query("SELECT * FROM tb_paket WHERE ',' + id_tb_lokasi + ',' LIKE '%,$id_lokasi,%' and is_hidden = '1' and nama_paket like '%carinya%'"); 
	}

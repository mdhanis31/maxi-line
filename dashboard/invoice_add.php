...
//bikin no invoice//
$sqlv = $db->fetchArray($db->query("SELECT MAX(id_tr_invoice) as idinvoice, max(no_invoice) as noinvoice from tr_invoice WHERE MONTH(tgl_invoice) = '$blnbesok'  and YEAR(tgl_invoice) = '$thbesok'"));
//echo $sqlv;
//exit;
...
if($tgl > 5){
$tglinvoice = $waktubesok ;
} else {
$tglinvoice = $now->format("05-m-Y");
}		
}

if(empty($tota) and $_GET['j'] == 2) {
$id_daftar = SafeSQL($_GET['id']);	
echo "<script>location.href='invoice_.php?id=$id_daftar&n=y&kd=$kd_invoice&j=2';</script>";
	   exit();	
...
if(in_array($_SESSION['level_user'], $arrLvlUser)) {
?>
...
$datetime1 = $ro['tgl_invoice'];
if($rowa['st_layanan'] == 7){
$datetime2 = $now; 
$difference = $datetime1->diff($datetime2);
$jeda = $difference->days;
} else {

$ss = $db->fetchArray($db->query("SELECT jumlah from tr_transaksi WHERE id_tr_invoice = '$ro[id_tr_invoice]' and jns_transaksi = '3'"));
$jeda = $ss['jumlah'];
}
...
//echo "jeda : ".$jeda."<br>";
//echo "biaya perhari : ".$biayahari."<br>";
//echo "tagihan : ".$haribiaya."<br>";
//echo "diskon : ".$diskon;
        ...
        <div class="box-body">
          <div class="row">
            ...
            <div class="col-md-6">
                   <div class="form-group">
                <div class="input-group">
                  <div class="input-group-addon" <?if($rowa['st_layanan'] == 7) {?> style="background-color:red;color:white;"  <?}?>>
                   Paket
                  </div>
				  <?if($rowa['st_layanan'] == 7) {?>
				  <script>alert('Cek kembali paket yang dipilih oleh customer, apakah paket promo / bukan!!')</script>
				  
				  
				  <form action="invoice_.php" method="post" enctype="multipart/form-data" name="form" id="formpaket">
				  ...
				  <select class="form-control" name="id_tb_paket" id="idpaket">
					<option value="<?=$rowa['id_tb_paket'];?>" selected><?=$sqlb['nama_paket'];?></option>
					<?
					$sql1 = "select * from tb_paket where id_paket_utama = '$rowa[id_tb_paket]'";
					$res1 = $db->query($sql1);		
					while($row1 = $db->fetchArray($res1)){?>					
					<option value="<?=$row1['id_tb_paket'];?>"><?=$row1['nama_paket'];?></option>
					<?}?>
					
					<?
					$sql2 = "select * from tb_paket where id_tb_paket = '$sqlb[id_paket_utama]'";
					$res2 = $db->query($sql2);		
					while($row2 = $db->fetchArray($res2)){?>					
					<option value="<?=$row2['id_tb_paket'];?>"><?=$row2['nama_paket'];?></option>
					<?}?>
					</select>
				  </form>	
				  <?} else {?>
				...
				<?if($rowa['st_layanan'] == 7) {?>
				  <p style="color:red;"><b>Cek apakah paket yang dipilih customer sudah sesuai (paket promo atau bukan)!!</b></p>
				<?}?>  
              </div>
				<!-- /.form-group -->
				<?php
      ...
      <!-- /.box -->
            <div class="box-body border-radius-none">
			  ...
			  <table id="example1" class="table table-bordered table-striped">
				...
				?>
                <tbody>
					<?php if(!empty($sqlb['id_paket_utama'])) { ?>
						<tr>
			...
			<div style="margin-top:50px;">	
			<?if($rowa['st_layanan'] == 7) {?>
			<button  class="btn btn-info" id="terbit">Aktifkan layanan</button>			
			<?} else if($ro['sts_invoice'] == 2){?>
			<a href="invoice_p.php?id=<?=maxiline($id_tr_invoice, 'e');?>&token=<?=$kodeaman?>"  target="_BLANK" class="btn btn-warning">Cetak Invoice</a>
			<?} else if($rowa['st_layanan'] == 8 and $ro['sts_invoice'] == 1){?>
			<form action="invoice_.php" method="post" enctype="multipart/form-data" name="form" id="form3" style="width: 20%; display: contents;">
	     ...
	     <input type="hidden" name="id_tr_invoice" value="<?=$id_tr_invoice;?>" >
		 <input type="hidden" name="tgl_invoice" value="<?=$tglinvoice;?>" id="tgl_invoice">
		 <input type="hidden" name="tgl_expired" value="<?=$tglexpired;?>" id="tgl_expired">
...
$('#terbit').click(function(){	
	if (!confirm('Simpan Invoice & aktifkan layanan?')) {

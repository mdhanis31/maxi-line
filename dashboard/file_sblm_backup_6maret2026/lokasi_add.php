...
.table {
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
	 <? if($_GET['f']=='d') {echo "Detil Lokasi";} else if($_GET['f']=='e') {echo "Edit Lokasi";} else {echo "Tambah Lokasi";}?>
		...
		<?}?>  
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Lokasi Tiang</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
			...
			<?}?>
              <div class="box-body">
			    ...
			    <? if($_GET['f']=='d') {
				  $rowa = $db->fetchArray($db->query("select * from v_alamat where id_data_kd_pos ='$row[id_v_alamat]'"));
				  if(empty($row['id_v_alamat'])){
				  $id_alamat = "-";
				  } else {
				  $id_alamat = $rowa['kelurahan_desa'].' - '.$rowa['kecamatan'].' - '.$rowa['kabupaten_kota'].' - '.$rowa['kd_pos'] ;
				  }	
				?>
			    <input type="text" class="form-control" value="<?=$id_alamat;?>" disabled>
			    <?} else if($_GET['f'] == 'e') {?>	
                <select class="form-control select2" id="id_alamat" name="id_alamat" style="width: 100%;" required > 
				<?$rowa = $db->fetchArray($db->query("select * from v_alamat where id_data_kd_pos ='$row[id_v_alamat]'"));
				  if(empty($row['id_v_alamat'])){
				  $id_alamat = "-";
				  } else {
				...
				?>
				 <option value="<?=$rowa['id_data_kd_pos'];?>" selected="selected"><?=$id_alamat;?></option>
				</select>
				<?} else {?>
				<select class="form-control select2" id="id_alamat" name="id_alamat" data-placeholder="Ketik kecamatan, kota atau desa" style="width: 100%;" required> 
				</select>
			   <?}?>
				</div>
				<p></p>

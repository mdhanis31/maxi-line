...
if ($kodeaman && $_POST['token']==$kodeaman) {
	...
	if ($jml == 0) {	
		...
		exit();
			...
			// insert database
			$sqla = "insert into tb_pendaftaran (kode_daftar, email, nama, telp, alamat, id_tb_lokasi, st_layanan, tipe_identitas, no_identitas, id_tb_paket) values ('$reg_code', '$email', '$nama', '$telp', '$alamat', '$id_tb_lokasi', '$st_layanan', '$tipe_identitas', '$no_identitas', '$id_tb_paket')";
			$resa = $db->query($sqla);

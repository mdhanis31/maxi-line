...
$sql = "select * from tb_perawatan where id_tb_perawatan = '$id_tb_perawatan'";
$res = $db->query($sql);
...
$tglperawatan = date_format($row['tgl_perawatan'], 'd-m-Y');	
$jamperawatan = date_format($row['tgl_perawatan'], 'H:i');
...
$sql = "select top 1 * from tb_rencana where id_tb_pendaftaran = '$id_tb_pendaftaran' and rencana = '4' and st_rencana = '1' order by tgl_rencana asc";
$res = $db->query($sql);
$row = $db->fetchArray($res);
...
$tglperawatan = date_format($row['tgl_rencana'], 'd-m-Y');	
$jamperawatan = date_format($row['tgl_rencana'], 'H:i');
}
}
      ...
      <div class="row">
		...
		<section class="col-lg-8">
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" action="perawatan_.php" method="post" enctype="multipart/form-data" name="form" id="formnya" role="form">
			...
			<input type="hidden" name="id_tb_perawatan" value="<?=$id_tb_perawatan;?>" />
			<?} else if($_GET['t']=='u') {?>
			<input type="hidden" name="f" value="e" />
			...
			<?}?>
              <div class="box-body">
				...
				<div class="form-group">
                  ...
                  <input type="text" class="form-control" id="datepicker" name="tglperawatan" value="<?=$tglperawatan;?>" readonly>
                </div>
				<div class="bootstrap-timepicker">
				<div class="form-group">
                  <label for="exampleInputFile">Waktu</label>
                  <input type="text" class="form-control timepicker" name="jamperawatan" value="<?=$jamperawatan;?>" <?if($_GET['s'] == "d") {echo "disabled";} else {echo "required";}?>>
                </div>	
				</div>
				...
				<div class="form-group">
                 ...
                 <select class="form-control select2" id="rencana" name="st_perawatan" data-placeholder="pilih hasil perawatan / perbaikan" style="width: 100%;" <?if($_GET['s'] == "d") {echo "disabled";} else {echo "required";}?>> 
				 <?if($_GET['s'] == "b") {?>
				 <option value="">--- Pilih Hasil ---</option>
				 <?}?>
				 <option value="1" <?if($row['st_perawatan'] == 1) {echo "selected";}?> >Pending</option>
				 <option value="2" <?if($row['st_perawatan'] == 2) {echo "selected";}?> >Selesai</option>
				 <option value="3" <?if($row['st_perawatan'] == 3) {echo "selected";}?> >Gagal</option>
				</select>
                </div>
				<div class="form-group">
                  <label for="exampleInputtext">Keterangan/alasan <?=$id_tb_perawatan;?></label>
				  <? if($_GET['s']=='d') {?>
                  <textarea type="textarea" class="form-control" id="keterangan" name="ket_perawatan" disabled placeholder="-"><?=$row['ket_perawatan'];?></textarea>
				  <?} else if($_GET['s']=='e') {?>
				  <textarea type="textarea" class="form-control" id="keterangan" name="ket_perawatan" placeholder="Masukkan keterangan" required><?=$row['ket_perawatan'];?></textarea>
				  <?} else {?> 
				  <textarea type="textarea" class="form-control" id="keterangan" name="ket_perawatan" placeholder="Masukkan keterangan" required></textarea>
                  <?}?>
				</div>
				...
				if($_GET['s']=='b') {
					...
					if($row['sts_perawatan'] == 1){?>
					<a href="perawatan_add.php?id=<?=maxiline($id_tb_perawatan, 'e');?>&s=e&t=u" id="olaole" class="btn btn-primary">Revisi</a>
					<?}
				} if($_GET['s']=='d') {
					if($row['sts_perawatan'] == 1){?>
					<a href="perawatan_add.php?id=<?=maxiline($id_tb_perawatan, 'e');?>&s=e" id="olaole" class="btn btn-primary">Revisi</a> 	
					<?}
				} else if($_GET['s']=='e') {
					if($row['sts_perawatan'] == 1){
						if(in_array($_SESSION['id_tb_user'], $id_usere) or $_SESSION['level_user'] == '1') {?>
				...
				if($_GET['t']=='u') {?>
				<button type="reset" class="btn btn-default" style="margin-left:5px;" onclick="window.location.href='perawatan_v.php'">Cancel</button>
				<?} else {?>
		...
		<div class="box box-primary">
            <div class="box-header with-border">
			  ...
			  <div class="box-tools">
				<?php if ($row['sts_perawatan'] != 2) {?>
						<a href="laporan_perawatan_add.php?id=<?=maxiline($id_tb_perawatan, 'e');?>&s=b&token=<?echo $kodeaman;?>" class="btn btn-sm btn-primary" style="margin-top:1px;"> 
			...
			<?
			$sqlx = "select * from tb_laporan where id_jns_laporan = '$id_tb_perawatan' and jns_laporan = '4'";
			$resx = $db->query($sqlx);
				...
				?>
					<li class="item">
						...
						<div class="product-info">
						  <a href="laporan_perawatan_add.php?id=<?=maxiline($rowx['id_tb_laporan'], 'e');?>&s=e" class="product-title"><?=$sqlz['nm_user']?>

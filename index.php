...
#cari:hover, #cari:hover::before, #cari:hover::after,
#reset:hover, #reset:hover::before, #reset:hover::after {
    background-color: #44b7c3 !important;
    border-color: #44b7c3 !important;
    color: #ffffff !important;
}

/* 1. Menetralkan zoom agar ukuran box & teks kembali ke skala 100% (1/0.95) */
.select2-container {
    zoom: 1.05263 !important; 
    width: 100% !important;
}
/* 2. Fix Dropdown position relative to form-group */
.form-group {
    position: relative;
}

span.select2-container.select2-container--open:not(.select2) {
    top: -2px !important; /* Pasang persis di dasar input box yang dinaikkan offset 40px karena margin-top */
    left: 15px !important; 
    width: calc(100% - 30px) !important;
    margin-top: 0 !important;
}

/* 3. Pastikan z-index tinggi agar tidak tertutup elemen lain */
.select2-container--open {
    z-index: 9999 !important;
}

/* 4. Perbaikan Visual: Pastikan box input tidak gepeng */
.select2-container .select2-selection--single {
    height: 38px !important; /* Sesuaikan dengan tinggi asli template Anda */
    display: flex;
    align-items: center;
}

/* Gabungkan class tanpa spasi untuk elemen yang sama */
span.select2.select2-container.select2-container--default {
    margin-top: -40px !important;
    padding: 0px 5px 0px 5px !important;
    display: block; /* Memastikan margin-top bekerja pada elemen span */
}

/* Jika Anda ingin padding merujuk pada teks di dalamnya agar lebih rapi */
span.select2.select2-container.select2-container--default .select2-selection--single {
    padding: 0px 5px !important;
    height: auto !important; /* Menyesuaikan jika box terlalu tinggi/pendek */
}


/* Menargetkan container utama agar tidak terdorong ke kanan */
span.select2.select2-container.select2-container--default {
    margin-left: 0 !important;
    left: 0 !important;
    display: block !important; /* Memaksa elemen mengisi ruang dengan benar */
    width: 100% !important;
}

/* Jika masih terdorong, gunakan margin negatif kecil untuk mengompensasi distorsi zoom */
span.select2.select2-container.select2-container--default {
    margin-left: -2px !important; /* Sesuaikan angka ini (misal -1px atau -3px) hingga lurus */
}

/* Pastikan box seleksinya sendiri tidak memiliki margin tambahan */
...
/* Menargetkan container Select2 saat elemen asli dinonaktifkan */
.select2-container--default.select2-container--disabled .select2-selection--single {
    background-color: #b3b7c4 !important;
    cursor: not-allowed;
}

/* Opsional: Menyesuaikan warna teks agar tetap terbaca di background baru */
	...
	<section class="pricing-section" id="pricing" style="background-image: url(resources/images/background/pattern-3.png)">
		<div class="auto-container">
			...
			<div class="sec-title light centered">
				<div class="title">Harga Paket</div>
				<h2>Paket Berlanganan Hanya Untukmu</h2>
				<div class="text"> Cari paket sesuai dengan wilayah anda atau hubungi kami untuk mendapatkan informasi harga maupun penawaran spesial yang berlaku di wilayah anda.</div>
			</div>
			<!-- End Sec Title -->
			<div class="row clearfix">
				...
				<div class="contact-form" style="width:100%;">
					<form method="post" action="index.php" id="contact-form">
						<div class="row clearfix">
							<div class="col-lg-3 col-md-12 col-sm-12 form-group" style="display:none;">
								<input type="hidden" name="cari" value="<?=maxiline('Y4', 'e');?>">
							</div>
							<div class="col-lg-3 col-md-12 col-sm-12 form-group">
								<select class="select2" name="provinsi" id="provinsi" data-placeholder="Cari Provinsi" required>
								</select>
							</div>
							<div class="col-lg-3 col-md-12 col-sm-12 form-group">
								<select class="select2" name="kabupaten" id="kabupaten" data-placeholder="Cari Kota/Kabupaten" required disabled>
								</select>
							</div>
							<div class="col-lg-3 col-md-12 col-sm-12 form-group">
								<select class="select2" name="kecamatan" id="kecamatan" data-placeholder="Cari Kecamatan/Kode Pos" required disabled>
								</select>
							</div>
							
							<div class="col-lg-3 col-md-12 col-sm-12 form-group text-center">
							<div class="row clearfix">
							<div class="col-lg-6 col-md-12 col-sm-12 form-group text-center" style="padding-right: 5px; padding-left: 5px;">
								<button class="theme-btn btn-style-four" id="cari" style="margin-top: var(--margin-top-0);padding: 10px 40px 10px; display:inline-flex; align-items:center; justify-content:center;" type="submit" name="submit-form">
									<span class="txt" style="gap:5px;"><i class="fa fa-search" aria-hidden="true" style="top: 0px;"></i> Cari</span>
								</button>
							</div>
							<div class="col-lg-6 col-md-12 col-sm-12 form-group text-center" style="padding-right: 5px; padding-left: 5px;">
								<button class="theme-btn btn-style-four" id="reset" style="margin-top: var(--margin-top-0);padding: 10px 40px 10px; display:inline-flex; align-items:center; justify-content:center;" type="button" name="reset-form">
									<span class="txt" style="gap:5px;"><i class="fa fa-times" aria-hidden="true" style="top: 0px;"></i> Reset</span>
								</button>
							</div>
							</div>
							</div>
				...
				<div id="search-result" class="row clearfix" style="width:100%; display:none; margin-top:30px;">
...
$('#reset').click( function(e) {
	...
	$('#kabupaten').prop('disabled', true).val(null).trigger('change');
	$('#kecamatan').prop('disabled', true).val(null).trigger('change');
...
$("#provinsi").on('select2:select', function (e) {
  $("#kabupaten").prop('disabled', false).val(null).trigger('change');
  $("#kecamatan").prop('disabled', true).val(null).trigger('change');
});
...
$("#kabupaten").on('select2:select', function (e) {
  $("#kecamatan").prop('disabled', false).val(null).trigger('change');
});
...
$('#contact-form').on('submit', function(e) {
    ...
    if (!prov ) {
        alert("Mohon lengkapi pilihan Provinsi terlebih dahulu.");
        return false;
    } else if (!kab) {
        alert("Mohon lengkapi pilihan Kota/Kabupaten terlebih dahulu.");
        return false;
    } else if (!kec) {
        alert("Mohon lengkapi pilihan Kecamatan terlebih dahulu.");
        return false;
    }

    $.ajax({
        url: 'search_coverage_ajax.php',
        type: 'POST',
        data: $(this).serialize(),
        success: function(response) {
            $('#search-result').html(response).show();
            // Re-initialize animations if wow.js is active
